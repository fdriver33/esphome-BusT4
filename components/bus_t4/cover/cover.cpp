...
void BusT4Cover::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  Init step: %u", init_step_);
  ESP_LOGCONFIG(TAG, "  Discovery attempts: %u", discovery_attempts_);
  ESP_LOGCONFIG(TAG, "  Target controller: 0x%02X.%02X", target_address_.address, target_address_.endpoint);
  ...
  ESP_LOGCONFIG(TAG, "  Operation: %s", bus_t4_cover_operation_to_str(current_operation));
  ESP_LOGCONFIG(TAG, "  Position: %.1f%%", this->position * 100.0f);
  if (init_ok_) {
